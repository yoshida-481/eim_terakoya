...
public class LumpUploadManager
{
	/** セッション **/
	...
	public LumpUploadManager(EIMSession _s, EIMObjectType _type, String _tmp, String _real) throws Exception
	{
		_sess = _s;
		_docObjType = _type;
		_tmpFolderPath = _tmp;
	...
	/**
	 * ZIPファイルの中身が登録可能かどうかのチェックを行う。
	 * @param root 一時格納フォルダのルート
	 * @param skipZipName チェック除外のZIPファイル名
	 * @param top ルートかどうかのフラグ
	 */
	public ArrayList recursiveCheckAssign(File root, boolean top)
	{
		...
		//再帰的処理
		if(top == false) {
			retList.add(checkAndCreateXMLData(root));
		}
		File[] list = FileUtil.sortedListFiles(root);
		for(int ii = 0; ii < list.length; ii++) {
			//自分がフォルダであろうがドキュメントであろうが先にチェックを行う
			//（ツリー構造のXMLを作成するため）
			retList.add(checkAndCreateXMLData(list[ii]));
			if(list[ii].isDirectory()) {
				//フォルダの場合は子を再帰的にチェック
				File[] children = FileUtil.sortedListFiles(list[ii]);
				for(int jj = 0; jj < children.length; jj++) {
	...
	/**
	 * 登録可否チェックし、クライアント返却用のXML文字列を作成する
	 * @param f チェック対象ファイル
	 */
	private LumpUploadXmlData checkAndCreateXMLData(File f)
	{
		//登録可否チェックを行った後、結果をデータクラスに格納する
		HashMap map = check(f);
		return new LumpUploadXmlData(f, getPath(f.getPath(), false), map.get(STATUS).toString(), map.get(REASON).toString());
	}
