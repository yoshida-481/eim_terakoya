...
<%
	...
	try
	{
		...
		EIMRole targetRole = RoleUtils.getRoleById(sess, Long.parseLong(prmRoleId));
		if (targetRole != null) {
			userList = RoleUtils.getUserList(sess, targetRole, 0, false);
		}
		...
		if(accessSecurityNarrowDownFlag && isFilter)
		{
			...
			// 表示ユーザ情報抽出
			List<EIMUser> accesableRoleUserList = new ArrayList<EIMUser>();
			List<Long> accesableRoleUserIdList = new ArrayList<Long>();
			for(EIMUser roleUser: userList)
			{
				if(accesableUserIdSet.contains(roleUser.getId()) && !accesableRoleUserIdList.contains(roleUser.getId()))
				{
					// EIMUserに変換しリストに追加
					accesableRoleUserList.add(roleUser);
					accesableRoleUserIdList.add((long)roleUser.getId());
				}
			}
			// セキュリティで絞り込んだユーザ情報のみ返却
			userList = accesableRoleUserList;
		}
