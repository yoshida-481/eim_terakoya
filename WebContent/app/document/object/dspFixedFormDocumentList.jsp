...
<%
	...
	class RecurrentUtils
	{
		...
		*/
		...
		public void write(EIMSession sess, FixedForm form, Map<Long, EIMObject> hasUpdateMap, String dataStr, JspWriter   out)
		throws Exception
		{
	...
	try{
		...
		String prmChangeFlag = request.getParameter("changeFlag");
		...
		// 絞込みフラグ取得
		EIMAttribute flagAttr = obj.getAttribute(EIMConfig.get("ATTR_NAME_WORKSPACE_LIMIT_DOCUMENT_TYPE_FLAG"));
		if(flagAttr != null && flagAttr.getInt() == 1)
		{
			// 絞込みフラグの値が1の時、使用可能ドキュメントタイプ属性の値と一致する
			// ドキュメントタイプの表示フラグをtrueにする(親、子含む)
			EIMAttribute docTypeAttr = obj.getAttribute(EIMConfig.get("ATTR_NAME_WORKSPACE_SELECTABLE_DOCUMENT_TYPE"));
			if(docTypeAttr != null){
				ArrayList<Long> idList = new ArrayList<Long>();
				long[] ids = TypeConvertUtils.convertToLongArray(docTypeAttr.getInts());
				for(long id : ids){
					idList.add(id);
				}
		...
		{
			// 絞込みフラグの値が1以外の時、すべての定型ドキュメントタイプの表示フラグをtrueにする
		...
		//ドキュメントタイプ選択画面の表示かどうか
		boolean changeFlag = Boolean.valueOf(prmChangeFlag).booleanValue();
		...
		if (changeFlag == true) {
			// 一般ドキュメント登録メニュー表示設定を取得
			String enableGeneralDocRegist = EIMConfig.get("DISPLAY_MENU_GENERAL_DOCUMENT");
			
			// 一般ドキュメント登録メニューを表示する場合、一般ドキュメントを返却
			if (!StringUtils.isBlank(enableGeneralDocRegist) && "true".equals(enableGeneralDocRegist)) {
				out.println("<objType" + "createFixedFormDocument" );
				out.println(" data=\"" + "createFixedFormDocument" + "\"" );
				out.println(" objTypeId=\"" + form.getFormType().getId() + "\"");
				out.println(" label=\"" + StringUtils.xmlEncode(form.getFormType().getName()) + "\"");
				out.println(">");
				out.println("</objType" + "createFixedFormDocument" + ">");
			}
		}
		...
		// 一般ドキュメント
		out.println("<objTypeGeneralDocument" );
		out.println(" data=\"generalDocument\"");
		out.println(" objTypeId=\"" + form.getFormType().getId() + "\"");
		out.println(" label=\"" + StringUtils.xmlEncode(form.getFormType().getName()) + "\"");
		out.println("/>");
		...
		ru.write(sess, form, hasUpdateMap, "createFixedFormDocument", out);
		ru.write(sess, form, hasUpdateMap, "joinFixedFormDocument", out);
		ru.write(sess, form, hasUpdateMap, "lumpUploadFixedFormDocument", out);
