	...
	try
	{
		...
		for (int i = 0; i < prmObjId.length; i++) {
			...
			if (!SecurityUtils.authorized(sess, object, sess.getUser(), EIMAccessRole.UPDATE)) {
				throw new EIMException(sess, "EIM.ERROR.LOGIC.NOACCESS.OCREXEC");
			}
			
			// OCR処理ステータスを取得
			long ocrProcStatus = AppOcrUtil.getOcrProcessingStatus(sess, object);
			
			if(ocrProcStatus != AppConstant.OCR_PROC_STATUS_PROCESSING){
				// OCR処理ステータスが「処理中」でない場合
				...
				// アクセス履歴
				AccessUtils.createAccess(sess, object, "EIM.ACCESS.TYPE.OCR.EXECUTE");
				
				// 操作履歴
				OperationHistoryUtils.create(sess, AppConstant.DOCUMENT, AppConstant.OCR_EXECUTE,
