	...
	try
	{
		...
		if (prmObjId == null || prmObjId.length() < 1) {
			...
			out.println("<object");
				...
				out.println(" dspObjTypeName=\"" + StringUtils.xmlEncode(label) + "\"");
				...
				// ドキュメントアクセスURL出力有無追加
				out.println(" docAccessUrl=\"" + eimFilePath + "\"");
				out.println(" originalDocAccessUrl=\"" + originalFilePath + "\"");
				out.println(" publicDocAccessUrl=\"" + publicFilePath + "\"");
				out.println(" publicFileName=\"" + "" + "\""); //公開ファイルが出力されないので名称は空文字
		...
		out.println("<object");
			...
			out.println(" url=\"" + StringUtils.xmlEncode(EIMConfig.get("DOCUMENT_URL") + EIMConfig.get("QUERY_STRING")) + "objId=" + object.getId() + StringUtils.xmlEncode("&") +"\"");
			out.println(" isDocument=\"" + isDocument + "\"");
			out.println(" isWorkSpace=\"" + isWorkSpace + "\"");
			out.println(" isTag=\"" + isTag + "\"");
			...
			//アクセス権限チェック
			if (isDocument) {
				// ドキュメントの場合は属性情報取得時に権限制御を行うため何もしない
			}
				...
				if (readOnly)
				{
					boolean hasAuth = AdminAuthUtil.hasSpecifiedOrGeneralAuth(user, AppConstant.ADMIN_AUTH_ID_SECURITY);
					if(!hasAuth)
					{
						readOnly = true;
					}
			...
			if (isDocument) {
				...
				if(isDispDirectURL){
					// 原本最新版URL
					out.println(" privateLatestURL=\"" + StringUtils.xmlEncode(EIMConfig.get("PRIVATE_LATEST_DOCUMENT_URL")) + "objId=" + object.getId() + StringUtils.xmlEncode("&") +"\"");
					// 公開最新版URL
					out.println(" publicLatestURL=\"" + StringUtils.xmlEncode(EIMConfig.get("PUBLIC_LATEST_DOCUMENT_URL")) + "objId=" + object.getId() + StringUtils.xmlEncode("&") +"\"");

					// ドキュメントアクセスURLも出す場合は公開ファイル名も取得する。

					EIMFormat format = FileUtils.getFormatByName(sess, EIMConfig.get("FORMAT_NAME_PUBLIC"));
					if (format != null) {
						EIMFile publicFile = FileUtils.getFile(sess, object, format);
