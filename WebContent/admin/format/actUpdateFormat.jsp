...
<%
	...
	try
	{
		...
		//通常のFormatServiceだと、ディレクトリも追加操作を行ってしまいエラーが発生してしまうため、Format処理のみのbeanを利用
		FormatService formatService = (FormatService)ApplicationContextLoader.getApplicationContext().getBean("formatServiceForFormatUpdate");

		//Format
		FormatDomain format = formatService.getById(Long.parseLong(prmFormatId));
		if(format == null)
		{
		...
		formatService.update(format); // 他言語名称は後続処理で設定する
		...
		for(int i = 0; i < prmOtherCnt; i++)
		{
			...
			//Update
			FileUtils.updateOtherFormatName(sess, format.getId(), prmOtherLId, prmOtherName);
		}

		// Create Operation History
		OperationHistoryUtils.create(sess, AppConstant.SYSTEM, EIMConstant.UPDATE_FORMAT,
				EIMConstant.TARGET_UPDATE, EIMConstant.FORMAT, ConvertUtils.toEIMFormat(format),
				null, null, null, null);
