 ...
 * @since		1.0.0
 */
public class ZIPDownloadPrivateDocuments extends AbstractZIPDownloadDocuments
{
	/**
	 * 
	 */
	...
	boolean checkAccessRight(EIMSession session, EIMObject object, AppObjectConditionHelper helper) throws Exception {
		...
		// 権限チェック
		boolean result = SecurityUtils.authorized(session, object, session.getUser(), EIMAccessRole.UPDATE);
		return result;
	}
