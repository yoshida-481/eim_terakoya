...
<%
	...
	try
	{
		...
		//Security
		EIMStatusSecurity sec = StatusSecurityUtils.getStatusSecurityById(sess, Long.parseLong(prmSecId));
		if(sec == null)
		{
		...
		//Delete
		StatusSecurityUtils.deleteStatusSecurity(sess, sec);
		
		// SearchFramework 検索FW更新通知 対象：セキュリティ
		AppUpdateNoticeUtils.updateNoticeInsert(sec.getDefaultSec().getId(), "SEARCHFW_SECURITY_DELSTATUS_SECURITY");
		
		//履歴追加
		OperationHistoryUtils.create(sess, AppConstant.SYSTEM, EIMConstant.DELETE_STATUS_SECURITY, 
		...
		out.println("<security secId=\"" + sec.getDefaultSec().getId() + "\"/>");
	}
