...
<%
	...
	// StatusTypeID
	String statusTypeId = request.getParameter("stTypeId");
	...
	try
	{
		...
		// デフォルトセキュリティを取得
		EIMSecurity defaultSec = SecurityUtils.getSecurityById(sess, Long.parseLong(defaultSecId));
		if(defaultSec == null)
		{
		...
		// ステータスタイプを取得
		EIMStatusType statusType = WorkFlowUtils.getStatusTypeById(sess, Long.parseLong(statusTypeId));
		if(statusType == null)
		{
		...
		// Create Security
		EIMStatusSecurity statusSec = StatusSecurityUtils.createStatusSecurity(sess, defaultSec, statusType);
		
		// SearchFramework 検索FW更新通知 対象：セキュリティ
		AppUpdateNoticeUtils.updateNoticeInsert(defaultSec.getId(), "SEARCHFW_SECURITY_CREATESTSEC_SECURITY");
		
		//履歴追加
		OperationHistoryUtils.create(sess, AppConstant.SYSTEM, EIMConstant.CREATE_STATUS_SECURITY, 
				EIMConstant.TARGET_CREATE, EIMConstant.SECURITY, statusSec,
				null, null, null, null );
		
		//出力XML
		out.println("<statusSecurity secId=\"" + statusSec.getId() + "\"/>");
