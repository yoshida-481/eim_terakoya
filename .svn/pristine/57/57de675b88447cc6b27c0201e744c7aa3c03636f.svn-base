	...
	try
	{
		...
		for (int i = 0; i < prmObjId.length; i++) {
			...
			if (!SecurityUtils.authorized(sess, object, sess.getUser(), EIMAccessRole.UPDATE)) {
				throw new EIMException(sess, "EIM.ERROR.LOGIC.NOACCESS.OCRCANCEL");
			}
			
			// OCR処理ステータスを取得
			long ocrProcStatus = AppOcrUtil.getOcrProcessingStatus(sess, object);
			
			if(ocrProcStatus == AppConstant.OCR_PROC_STATUS_PROCESS_WAIT){
				// OCR処理ステータスが「処理待ち」の場合
				
				// OCR処理ステータスを更新する
				AppOcrUtil.setOcrProcessingStatus(sess, object, AppConstant.OCR_PROC_STATUS_NONE);
				
				// OCR処理オブジェクトを削除する
				AppOcrUtil.deleteOcrProcessingObject(sess, object);
				
				// アクセス履歴
				AccessUtils.createAccess(sess, object, "EIM.ACCESS.TYPE.OCR.CANCEL");
				
				// 操作履歴
				OperationHistoryUtils.create(sess, AppConstant.DOCUMENT, AppConstant.OCR_CANCEL,
