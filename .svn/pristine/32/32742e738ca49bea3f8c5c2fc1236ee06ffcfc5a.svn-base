...
%><c:set var="resultDataList" value="${resultDataList}"

/><?xml version="1.0" encoding="UTF-8"?>
<result>
	<type>
		<c:out value="${resultDataList.type}" escapeXml="true"/>
	</type>
	<code>
		<c:out value="${resultDataList.code}" escapeXml="true"/>
	</code>
	<message>
		<c:out value="${resultDataList.message}" escapeXml="true"/>
	</message>
	
	<data
		offset="<c:out value="${resultDataList.offset}" escapeXml="true"/>"
	>
		<c:forEach items="${resultDataList.targetList}" var="resultData">
			<object>
				<id>
				...
				<c:if test="${resultData.noWFStatus==0 || resultData.noWFStatus==1 || resultData.noWFStatus==2}" >
					<NoWFDocStatus>
						<c:out value="${resultData.noWFStatus}" escapeXml="true" />
					</NoWFDocStatus>
				</c:if>
				<update 
				...
				<c:if test="${resultDataList.isDispLink==true}">
					<link>
						<c:out value="${resultData.isLinked}" escapeXml="true"/>
					</link>
				</c:if>
				...
				<c:if test="${resultDataList.isDispAttr==true}">
					<attList>
