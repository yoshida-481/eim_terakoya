...
<%
	...
	String[] prmNewCopyFlags = request.getParameterValues("newCopyFlag");
	...
	try
	{
		...
		int i = 0;
		for (String attTypeId : prmAttTypeIds) {
			//boolean prmnewCopyFlags = true;
			boolean prmnewCopyFlags = Boolean.valueOf(prmNewCopyFlags[i]);
			attTypeLayoutDomain = new AttributeTypeLayoutDomain();
			attTypeLayoutDomain.setId(Long.parseLong(attTypeId));
			attTypeLayoutDomain.setNewCopyFlag(prmnewCopyFlags);
		...
		objectTypeLayoutService.newCopyAttributeType(objTypeLayoutDomain);

		// 履歴生成
		OperationHistoryUtils.create(sess, AppConstant.SYSTEM, AppConstant.NEWCOPY_CLASS_ATTRIBUTE,
