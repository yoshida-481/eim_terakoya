CREATE OR REPLACE TRIGGER delete_workspace_trg_before
                  BEFORE DELETE
                  ON EIMREL
                  FOR EACH ROW
DECLARE
...
BEGIN
	...
	if :old.type != doc_rel_type and :old.type != link_rel_type then
	...
	select count(id) into data_count from EIMOBJ where id = :old.parent and type = ws_obj_type;
	...
	select count(value) into data_count from EIMOBJINT where id = :old.parent and type = ws_attr_type;
