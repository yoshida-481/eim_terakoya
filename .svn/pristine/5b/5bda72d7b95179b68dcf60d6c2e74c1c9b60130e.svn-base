...
<%
		...
		throws Exception
		{
			for (FixedForm child : form.getChildType()) {
				...
				if ((child.getFormType().getSecurity() == null ||
					...
					out.println("<objType");
					out.println(" data=\"createFixedFormTag\"");
	...
	try{
		...
		// 絞込みフラグ取得
		EIMAttribute flagAttr = obj.getAttribute(EIMConfig.get("ATTR_NAME_WORKSPACE_LIMIT_TAG_TYPE_FLAG"));
		if(flagAttr != null && flagAttr.getInt() == 1)
		{
			// 絞込みフラグの値が1の時、使用可能タグタイプ属性の値と一致するタグタイプの表示フラグをtrueにする(親、子含む)
			EIMAttribute tagTypeAttr = obj.getAttribute(EIMConfig.get("ATTR_NAME_WORKSPACE_SELECTABLE_TAG_TYPE"));
			if(tagTypeAttr != null){
				ArrayList<Long> idList = new ArrayList<Long>();
				long[] ids = TypeConvertUtils.convertToLongArray(tagTypeAttr.getInts());
				for(long id : ids){
					idList.add(id);
				}
		...
		{
			// 絞込みフラグの値が1以外の時、すべての定型タグタイプの表示フラグをtrueにする
		...
		// Start Root Node
		out.println("<objTypeList generalTagId='" + objType.getId() + "'>");
