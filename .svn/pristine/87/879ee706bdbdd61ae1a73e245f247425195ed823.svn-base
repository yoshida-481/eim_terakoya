...
<%
	...
	try
	{
		...
		List<EIMUser> tmpUserList = new ArrayList<EIMUser>();
		for(int i = 0; i < userList.size(); i++)
		{
			EIMUser targetUser = (EIMUser)userList.get(i);
			if(userSet.contains((long)targetUser.getId())) continue; 
			
			userSet.add((long)targetUser.getId());	
			tmpUserList.add(targetUser);
		}
		...
		for(int i = 0; i < userList.size(); i++)
		{
			...
			{
				EIMRole rl = (EIMRole)roleList.get(j);
				if(j == 0)
				{
					roleName += rl.getName();
				}
