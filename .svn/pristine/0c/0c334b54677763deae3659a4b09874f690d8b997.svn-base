...
<%
	...
	try
	{
		...
		//Update Attribute Tree
		int prmOtherCnt = LanguageFieldUtil.getOtherCnt(request);
		String prmDefName = LanguageFieldUtil.getDefName(request, prmOtherCnt);

		//Update Attribute Tree
		AttributeTreeUtil.updateAttributeTree(sess, attributeTree, prmDefName, Integer.parseInt(prmClassifyTarget));

		//Update Attribute Tree Other
		for(int i = 0; i < prmOtherCnt; i++)
		{
			...
			//Update Attribute Tree
			AttributeTreeUtil.updateOtherAttributeTreeName(sess, Long.parseLong(prmAttrTreeId), prmOtherLId, prmOtherName);
		}
		
		//Create Operation History
		AttributeTreeUtil.createOperationHistory(sess, 
				                                 AppConstant.SYSTEM, 
				                                 EIMConstant.UPDATE_ATTRIBUTE_TREE_VIEW,
