 ...
 * パラメータ変換プラグインの実装クラス
 * <br>
 * 入力値として添付ファイルを指し示すFileAccessDomainを受け取り、それが添付されているドキュメントのファイルパスを取得するプラグインクラスです。
 * <br>
 * @since Ver6.6
 */
public class FileAccessToFilePathConverterImpl implements ParameterConverterPlugIn {
	 ...
	 * パラメータ変換の処理。<br>
	 * <br>
	 * FileAccessDomainを受け取り、それが添付されているドキュメントのファイルパスを返却します。
	 * <br>
	...
	public Object convert(Object source) throws Exception {
		...
		FileAccessDomain fileAccess = (FileAccessDomain)source;
		
		// ファイルのオブジェクトタイプ定義名称
		String fileObjTypeDefName = fileAccess.getObject().getType().getDefinitionName();
		...
		if(fileObjTypeDefName.equals(ConfigUtils.getByKey("OBJECT_TYPE_NAME_TEMP_ATTACH_FILE"))) {
			...
			long fileObjId = fileAccess.getObject().getId();
