...
/** */
public class DspAttrTreeTest2 extends JSPTestCase {
	/**
	 * 
	 * @return o
	 * @throws Exception
	...
	public static Test suite() throws Exception {
		TestSuite suite = new TestSuite(DspAttrTreeTest2.class);
	 ...
	 * @throws Exception
	 */
	public void testDocumentClassify() throws Exception {
		ds.switchUser("tu1");

		// TestCase:掘り下げ1段
		// 検証用雛形XMLを作成
		String assertBaseText = Misc.loadFile(this.getClass().getResource(
			ClassUtils.getShortClassName(this.getClass()) + "d_01.txt").getPath());
		...
		assertBaseText = Misc.loadFile(this.getClass().getResource(
			ClassUtils.getShortClassName(this.getClass()) + "d_02.txt").getPath());
		assertText = TestAttrTreeUtil.replaceAssertText(sess, assertBaseText, attrt1, attrt2, null);
		// JSP返却結果XMLを検証
		assertEquals(assertText, xu.toStr(xu.toDOM(ds.attrTree_dspAttrTree(attrt2.getId(),
			new String[] { "b" }, attrt2settings))));

		// TestCase:ドキュメント分類の属性ツリーに末端を要求
		assertBaseText = Misc.loadFile(this.getClass().getResource(
			ClassUtils.getShortClassName(this.getClass()) + "d_03.txt").getPath());
		assertText = TestAttrTreeUtil.replaceAssertText(sess, assertBaseText, attrt1, attrt2, null);
		assertEquals(assertText, xu.toStr(xu.toDOM(ds.attrTree_dspAttrTree(attrt2.getId(),
			new String[] { "b", "う\nえ" }, attrt2settings))));
	}
	 ...
	 * @throws Exception
	 */
	public void testFolderClassify() throws Exception {
		ds.switchUser("tu1");
		...
		String assertText = TestAttrTreeUtil.replaceAssertText(sess, assertBaseText, attrt1,
			attrt2, null);
		// JSP返却結果XMLを検証
		assertEquals(assertText, xu.toStr(xu.toDOM(ds.attrTree_dspAttrTree(attrt1.getId(),
			new String[] { "2" }, attrt1settings))));

		// TestCase:掘り下げ2段(ツリー1/ツリー2[末端])
		// 検証用雛形XMLを作成
		assertBaseText = Misc.loadFile(this.getClass().getResource(
		...
		// TestCase:掘り下げ3段(ツリー1/ツリー2[末端]/フォルダ)
		// 検証用雛形XMLを作成
		assertBaseText = Misc.loadFile(this.getClass().getResource(
			ClassUtils.getShortClassName(this.getClass()) + "_03.txt").getPath());
		assertText = TestAttrTreeUtil.replaceAssertText(sess, assertBaseText, attrt1, attrt2, objs);
		EIMObject obj = TestAppObjectUtil.findObj("F21", objs);
		// JSP返却結果XMLを検証
		assertEquals(assertText, xu.toStr(xu.toDOM(ds.attrTree_dspAttrTree(attrt1.getId(),
			new String[] { "2", "1025449200000", String.valueOf(obj.getId()) }, attrt1settings))));

		// TestCase:「属性値なし」の掘り下げ
		assertBaseText = Misc.loadFile(this.getClass().getResource(
		...
		// TestCase:「属性値なし」の掘り下げ3段
		assertBaseText = Misc.loadFile(this.getClass().getResource(
			ClassUtils.getShortClassName(this.getClass()) + "_06.txt").getPath());
		assertText = TestAttrTreeUtil.replaceAssertText(sess, assertBaseText, attrt1, attrt2, objs);
		// JSP返却結果XMLを検証
		assertEquals(assertText, xu.toStr(xu.toDOM(ds.attrTree_dspAttrTree(attrt1.getId(),
			new String[] { "", "", String.valueOf(TestAppObjectUtil.findObj("F5", objs).getId()) },
			attrt1settings))));
	}
	...
	public void testAbnormal() throws Exception {
		...
		ds.br.eimLogout();
		try {
			ds.attrTree_dspAttrTree(-1, null, null);
			fail();
		...
		try {
			ds.attrTree_dspAttrTree(1234, null, null);
			fail();
		} catch (EIMServerResponseError e) {
		...
		sess.commit();
		try {
			ds.attrTree_dspAttrTree(attrt1.getId(), null, null);
			fail();
		} catch (EIMServerResponseError e) {
		...
		ds.attrTree_dspAttrTree(attrt1.getId(), null, attrt1settings);// 念のため。エラーは発生しない。
		...
		sess.commit();
		try {
			ds.attrTree_dspAttrTree(attrt1.getId(), null, attrt1settings);
			fail();
		} catch (EIMServerResponseError e) {
	...
	public void testAbnormal2() throws Exception {
		...
		try {
			ds.attrTree_dspAttrTree(attrt1.getId(), new String[] { "999", }, attrt1settings);
			fail();
		} catch (EIMServerResponseError e) {
			assertEquals(EIMResource.getMessage("JA",
				"EIM.ERROR.LOGIC.ATTRTREE.NOTEXIST.ATTR.VALUE"), e.getMessage());
		}

		// TestCase:属性ツリーの2段目が最新では変更
		try {
			ds.attrTree_dspAttrTree(attrt1.getId(), new String[] { "2", "999" }, attrt1settings);
			fail();
		} catch (EIMServerResponseError e) {
			assertEquals(EIMResource.getMessage("JA",
				"EIM.ERROR.LOGIC.ATTRTREE.NOTEXIST.ATTR.VALUE"), e.getMessage());
		...
		try {
			ds.attrTree_dspAttrTree(attrt1.getId(), new String[] { "2", "1025449200000", "999" },
				attrt1settings);
			fail();
		} catch (EIMServerResponseError e) {
			assertEquals(EIMResource.getMessage("JA", "EIM.ERROR.LOGIC.NOWSFOL"), e.getMessage());
		}

		// TestCase:属性ツリーの3段目のフォルダが最新では別の位置
		try {
			ds.attrTree_dspAttrTree(attrt1.getId(), new String[] { "2", "1025449200000",
