 ...
 * 属性ツリービュー更新クラスのテスト
 */
public class ActUpdateAttrTreeTest extends TestCase {
	/** */
	AdminService as;
	...
	public void testMe() throws Exception {
		...
		// 更新パーラーメータ
		String ja_nameNew = "langNameNew";
		String en_nameNew = "en:langNameNew";
		int classifyTargetNew = 1;
		String[][] otherLangNamesNew = { { "JA", ja_nameNew }, { "EN", en_nameNew } };

		as.attrTree_actUpdateAttrTree(attributeTree.getId(), otherLangNamesNew, classifyTargetNew);

		attributeTreelist = AttributeTreeUtil.getAttributeTreeList(sess);
		assertEquals(1, attributeTreelist.size());
		attributeTree = (AttributeTree) attributeTreelist.get(0);
		assertEquals(ja_nameNew, attributeTree.getName());
		assertEquals(classifyTargetNew, attributeTree.getClassifyTarget());
	...
	public void testAbnormal_1() throws Exception {
		...
		try {
			...
			int classifyTarget = 0;
			String[][] otherLangNames = { { "JA", ja_name }, { "EN", en_name } };
			as.attrTree_actUpdateAttrTree(1, otherLangNames, classifyTarget);
