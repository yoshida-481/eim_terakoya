 ...
 * を拡張している。
 */
public class DownloadPublicLatestDocument extends AbstractDownloadLatestDocument
{
	...
	boolean checkAccessRight(EIMSession session, EIMObject object, AppObjectConditionHelper helper) throws Exception {
		...
		if(!SecurityUtils.authorized(session, object, session.getUser(), EIMAccessRole.READ)){
			// 公開読取権限がない場合、権限なし
			return false;
		}
