...
<%
//-------------------------------------------------------------------------
//ワークスペース管理機能追加のため、app/object/dspAtribute.jspをコピーし
//追加修正
//※ワークスペースの属性情報取得の際によばれる
//														(2012/02/20)
//-------------------------------------------------------------------------
...
//---ワークスペースは親オブジェクトを持たないためコメントアウト
//			if (prmParentObjId != null && prmParentObjId.length() > 0) {
//				//親オブジェクト取得
//				parentObj = ObjectUtils.getObjectById(sess, Integer.parseInt(prmParentObjId));
...
//					int[] parentLowAttrs = AppObjectUtil.getIntAttrs(sess,
...
//			}
			...
			for(int i = 0; i < attTypeList.size(); i++)
			{
				...
				//非表示の属性は返却しない
				if(excludeAttrNames.contains(attrName)){
				     continue;
				}
...
//			}
		...
		if(object == null || hasDspAttrRole == false)
		{
