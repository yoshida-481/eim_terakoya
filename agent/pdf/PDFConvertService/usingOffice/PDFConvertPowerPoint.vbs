...
function main()
	...
	On Error Resume Next
		...
		Dim app: Set app=WScript.CreateObject("PowerPoint.Application")
		'app.Visible=False
		app.DisplayAlerts = False
		
		Dim book: Set book = app.Presentations.Open(origFile&"::-1", ,True, False)

		checkError()
		if (Not ErrorFlg and book.Slides.Count <> 0) then
			Dim xlTypePDF: xlTypePDF = 32
			Call book.SaveAs(pdfFile, xlTypePDF)
			checkError()
		elseif (book.Slides.Count = 0) then
			' PowerPointはスライドを全て削除できる。その場合エクスポートできないためエラー扱いとする。
			ErrorFlg = True
			WScript.StdErr.WriteLine(logMessage("ERROR", "[ErrorCode=" & -1 & "]" & "印刷に指定したスライドは、存在しません。"))
		end if
