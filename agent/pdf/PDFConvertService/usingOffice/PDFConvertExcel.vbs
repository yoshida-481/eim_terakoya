...
function main()
	...
	On Error Resume Next
		...
		Dim app: Set app=WScript.CreateObject("Excel.Application")
		app.Visible=False
		app.DisplayAlerts = False
		app.EnableEvents  = False
		
		Dim book: Set book = app.Workbooks.Open(origFile,,True,,"-1")
		checkError()

		if (Not ErrorFlg) then
			Dim xlTypePDF: xlTypePDF = 0
			Call book.ExportAsFixedFormat(xlTypePDF, pdfFile)
			checkError()
		end if
